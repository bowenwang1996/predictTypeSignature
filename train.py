...
from utils import *
from prepare_data import start_token, end_token, prepareData, prepareDataWithFileName
...
def train(data, model, optimizer, **kwargs):
    epoch_loss = 0.0
    start = time.time()
    model.train()
    ...
    for i, batch in enumerate(data):
        ...
        loss.backward()
        if kwargs["grad_clip"] > 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), kwargs["grad_clip"])
        optimizer.step()
        epoch_loss += loss.item()/model.train_batch_size
        if (i+1) % 1000 == 0:
            checkpoint_num = (i+1) // 1000 + kwargs["checkpoint_base"]
            epoch = kwargs["epoch"]
            kwargs["logger"].add_scalar("avg loss", epoch_loss/(i+1), epoch * 12 + checkpoint_num)
            print("checkpoint{} avg loss: {:.4f}".format(checkpoint_num, epoch_loss/(i+1)))
            print("time since start: {}".format(timeSince(start)))
            manager = kwargs["manager"]
            manager.torch_save({"epoch": epoch,
                                "checkpoint": checkpoint_num,
                                "model_state": model.state_dict(),
                                "optimizer_state": optimizer.state_dict(),
                                "best_acc": kwargs["best_accuracy"],
...
def eval(data, model, is_test=False):
    ...
    for batch in data:
        ...
        for i in range(batch_size):
            _, sig, _ = batch[i]
            predict_sig = tokensToString(decoded_tokens[i].tolist(), model.batch.target_vocab, trainInfo.idx_oov_dict)
            #print("prediction: {}".format(predict_sig))
            #print("sig: {}".format(process_sig(sig)))
    ...
    parsable_percent = num_parsable/(data_len * batch_size)
    if is_test:
        return accuracy, structural_acc, parsable_percent
    return eval_loss/data_len, accuracy, structural_acc, parsable_percent
...
def tokensToString(tokens, lang, idx_oov_dict):
    if tokens[-1] == end_token:
        tokens = tokens[:-1]
    tokens = takewhile(lambda x: x > 0, tokens)
    s = ""
    for token in tokens:
        if token in lang.idx_to_type:
            s += lang.idx_to_type[token] + " "
...
def randomEval(data, model):
    ...
    decoded_tokens = model(trainInfo, is_test=True)
    decoded_token = decoded_tokens[0].tolist()
...
def main(arg):
    ...
    if arg.use_qualified_name == 1:
        input_lang, output_lang, train_data = prepareDataWithFileName(arg.train_data_qualified, use_context=use_context)
        _, _, dev_data = prepareDataWithFileName(arg.dev_data_qualified, use_context=use_context)
        _, _, test_data = prepareDataWithFileName(arg.test_data_qualified, use_context=use_context)
    else:
    ...
    train_data = [model.batch.variableFromBatch(x) for x in model.batch.batchify(train_data)]
    ...
    for epoch in range(epoch_start, arg.num_epoch):
        try:
            ...
            dev_loss, accuracy, structural_acc, parsable_percent = eval(dev_data, model)
            ...
            print("dev loss: {:.4f} accuracy: {:.4f} structural accuracy: {:.4f} parsable percent: {:.4f}".format(dev_loss, accuracy, structural_acc, parsable_percent))
    ...
    test_accuracy, test_structural_acc, parsable_percent = eval_test(test_data, model)
    #writer.add_scalar("Test acc", test_accuracy)
    #writer.add_scalar("Test structural acc", test_structural_acc)
    print("test accuracy: {:.4f} test structural accuracy: {:.4f} parsable percent: {:.4f}".format(test_accuracy, test_structural_acc, parsable_percent))
